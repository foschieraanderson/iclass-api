...
const GENERIC_RESPONSE = { message: 'Se o email estiver cadastrado, você receberá um código de recuperação.' }
...
export class PasswordResetService {
  ...
  async requestPasswordReset(data: ForgotPasswordDTO) {
    ...
    if (user) {
      ...
      await this.app.sendEmail(
        ...
        `<p>Seu código de recuperação é: <strong>${code}</strong></p><p>Ele expira em ${EXPIRATION_MINUTES} minutos.</p>`
      )
    }
  ...
  async resetPassword(data: ResetPasswordDTO) {
    ...
    const token = await tokenRepository.findActiveByUserIdAndCode(user.id, data.code)
